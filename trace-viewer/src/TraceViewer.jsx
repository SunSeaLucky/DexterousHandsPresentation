  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to load ${tracePath}: ${response.status} ${response.statusText}`);
        }
...
function renderLines({trace, currentPath, currentLineNumber, currentStepIndex, targetStepIndex, rawMode, animateMode, navigate}) {
  ...
  // Build a map of location to renderings. Sections can share line numbers,
  // so the path has to be part of the key.
  const locationToRenderings = {};
  for (const step of trace.steps) {
    const stackElement = getLast(step.stack);
    locationToRenderings[getLocation(stackElement.path, stackElement.line_number)] = step.renderings;
  }
  ...
  const renderedLines = lines.map((line, index) => {
    ...
    const renderings = locationToRenderings[location];
    const renderedItems = [];
...
function gotoLine({trace, currentPath, currentLineNumber, currentStepIndex, lineNumber, navigate}) {
  ...
  if (currentLineNumber <= lineNumber) {
    ...
    while (stepIndex < trace.steps.length) {
      ...
      if (stackElement.path === currentPath && stackElement.line_number === lineNumber) {
